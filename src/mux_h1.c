...
static void h1_set_cli_conn_mode(struct h1s *h1s, struct h1m *h1m)
{
	...
	if (h1m->flags & H1_MF_RESP) {
		...
		if (!(h1m->flags & (H1_MF_VER_11|H1_MF_CONN_KAL)) || h1m->flags & H1_MF_CONN_CLO || (h1s->meth == HTTP_METH_CONNECT))  {
...
static size_t h1_process_input(struct h1c *h1c, struct buffer *buf, size_t count)
{
	...
	do {
		...
		if (h1m->state <= H1_MSG_LAST_LF) {
			...
			if (h1m->flags & H1_MF_CONN_UPG && h1m->state != H1_MSG_DONE) {
	 ...
	 */
	if (h1m->state == H1_MSG_DONE  && !(h1m->flags & H1_MF_CONN_UPG))
		...
		switch (h1m->state) {
			...
			case H1_MSG_LAST_LF:
			  ...
			  last_lf:
				...
				if (!(h1m->flags & H1_MF_RESP) && h1s->meth == HTTP_METH_CONNECT) {
					...
					if (htx_is_last_blk(chn_htx, blk) && (chn_htx->flags & HTX_FL_EOM)) {
			...
			case H1_MSG_TUNNEL:
				...
				if (h1m->state == H1_MSG_DATA)
					...
					TRACE_PROTO((!(h1m->flags & H1_MF_RESP) ? "H1 request tunneled data xferred" : "H1 response tunneled data xferred"),
						    H1_EV_TX_DATA|H1_EV_TX_BODY, h1c->conn, h1s, 0, (size_t[]){v.len});
				if (last_data && h1m->state == H1_MSG_DATA)
					goto done;
				break;
